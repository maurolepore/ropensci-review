...
### Fase 4: Búsqueda de Revisores (~2 semanas)
1. **Editor/a** pide a **Autor/a** que sugiera 3 revisores potenciales (puede usar 0-1 de ellos)
2. **Editor/a** busca 2 revisores de:
   - Base de datos de revisores voluntarios
   - Autores de dependencias del paquete o paquetes dependientes
   - Personas activas en dominios relevantes
   - Sugerencias del autor (usar con moderación)
3. **Editor/a** contacta a revisores potenciales (vía email o GitHub)
4. **Editor/a** usa la [plantilla de solicitud de revisión](https://devguide.ropensci.org/es/reviewrequesttemplate.es.html)
5. **Revisores potenciales** verifican [conflictos de interés](https://devguide.ropensci.org/es/softwarereview_policies.es.html#coi) usando la guía de COI
6. **Revisores** aceptan o declinan en unos días
7. **Editor/a** asigna revisores con: `@ropensci-review-bot assign @username as reviewer`
8. **Bot** automáticamente:
   - Añade al revisor a la lista de revisores
   - Establece fecha límite (típicamente 3 semanas)
   - Publica mensaje de bienvenida con enlace a la [guía de revisores](https://devguide.ropensci.org/es/softwarereview_reviewer.es.html)
   - Pide al revisor que complete el formulario de revisores (Airtable)
   - Actualiza etiquetas del issue cuando se asignan 2 revisores
9. **Editor/a** puede modificar fechas límite con: `@ropensci-review-bot set due date for @username to YYYY-MM-DD`
10. **Gerente de Comunidad** invita a **Revisores** al Slack de rOpenSci

### Fase 5: Revisión (~2-3 semanas por revisor)
1. **Revisores** examinan el paquete independientemente
2. **Revisores** usan la [**plantilla de revisión**](https://devguide.ropensci.org/es/reviewtemplate.es.html) como lista de verificación
3. **Revisores** verifican:
...
5. **Revisores** publican sus revisiones como comentarios en el issue de GitHub
6. **Editor/a** registra cada revisión con: `@ropensci-review-bot submit review <url> time <hours>`
7. **Bot** registra las horas de revisión de cada revisor
8. **Revisores** pueden hacer pull requests con correcciones (opcional, ~20% lo hace)

### Fase 6: Respuesta del Autor (~2-3 semanas)
...
6. **Autor/a** comenta en el issue cuando esté listo para re-revisión

### Fase 7: Iteración (Variable)
1. **Revisores** verifican las respuestas y actualizaciones del autor
2. **Revisores** pueden solicitar cambios adicionales
3. **Autor/a** hace más actualizaciones
4. **Proceso se repite** hasta que los revisores estén satisfechos
5. **Sin rechazo** - el proceso continúa hasta que el paquete cumpla con los estándares

### Fase 8: Aprobación
1. **Revisores** usan la [plantilla de aprobación](https://devguide.ropensci.org/es/approval2template.es.html) para aprobar formalmente
2. **Revisores** publican comentarios de aprobación
...
5. **Bot** automáticamente publica lista comprensiva de TAREAS PENDIENTES para el autor incluyendo:
   ...
   - Instrucciones de reconocimiento a revisores
...
1. **Autor/a** completa la lista de verificación de TAREAS PENDIENTES:
   ...
   - [ ] Opcionalmente reconocer a revisores como "rev" en DESCRIPTION
...
- **Colaborativo**: Autores y revisores participan en diálogo
- **Altamente Automatizado**: El bot maneja verificaciones, recordatorios, asignaciones, registro y transferencia
- **Dirigido por Voluntarios**: Editores y revisores son voluntarios no remunerados
...
- **Email** (reclutamiento de revisores)
...
| `@ropensci-review-bot approve <paquete>` | Editor | Aprobar paquete y publicar lista de TAREAS |
| `@ropensci-review-bot invite me to ropensci/<paquete>` | Autor | Renovar invitación de transferencia expirada |
| `@ropensci-review-bot finalize transfer of <paquete>` | Autor | Completar transferencia del repositorio |

## Plantillas Clave Usadas:
- [**Plantilla de envío**](https://github.com/ropensci/software-review/issues/new/choose) - Usada por autores al crear el issue
- [**Plantilla de editor**](https://devguide.ropensci.org/es/editortemplate.es.html) - Usada por editores para verificaciones iniciales
- [**Plantilla de solicitud de revisión**](https://devguide.ropensci.org/es/reviewrequesttemplate.es.html) - Usada por editores para invitar revisores
- [**Plantilla de revisión**](https://devguide.ropensci.org/es/reviewtemplate.es.html) - Usada por revisores para realizar su revisión
- [**Plantilla de aprobación**](https://devguide.ropensci.org/es/approval2template.es.html) - Usada por revisores para aprobar formalmente
- **Comentario de aprobación del editor** - Usado por editores para aprobación final del paquete
...
- Solicitudes de formulario de revisores
